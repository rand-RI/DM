...
CS_Exhibit5_USEquities= srm.Correlation_Surprise(Returns=Exhibit5_USEquities)  # CS returns for USEquities
CS_Exhibit5_EuropeanEquities= srm.Correlation_Surprise(Returns=Exhibit5_EuropeanEquities)# CS returns for EuropeanEquities
CS_Exhibit5_Currency= srm.Correlation_Surprise(Returns=Exhibit5_Currency)      # CS returns for Currency
...
#Step2: Partition the sample from step 1 into two smaller subsamples: days with high correlation surprise and days with low correlation surprise    
        ...
        for i in range(len(MagSur_20)):                            
            ...
            for j in range(len(Correlation_Measures[k][2])):            
                    if x==Correlation_Measures[k][2].index[j]:                
